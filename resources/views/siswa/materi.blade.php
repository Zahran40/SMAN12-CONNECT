...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 flex flex-col items-center">
            <h4 class="font-semibold text-blue-500 mb-4">{{ $jadwal->mataPelajaran->nama_mapel ?? 'Mata Pelajaran' }}</h4>
            ...
            <p class="text-sm text-slate-600 mb-4">{{ $jadwal->kelas->nama_kelas ?? '' }}</p>
            <a href="{{ route('siswa.detail_materi', $jadwal->id_jadwal) }}" class="bg-blue-400 text-white font-medium py-2 px-12 rounded-full hover:bg-blue-700 transition-colors">
                Lihat
            </a>
        </div>
        @empty
