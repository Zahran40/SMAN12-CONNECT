    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        @forelse($jadwalMengajar as $group)
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 flex flex-col items-center text-center hover:shadow-xl transition-shadow border-2 border-transparent hover:border-blue-300">
                ...
                <h3 class="font-bold text-xl text-slate-800 mb-1">{{ $jadwal->mataPelajaran->nama_mapel }}</h3>
                <p class="text-sm text-slate-500 mb-4">{{ $jadwal->kelas->nama_kelas }}</p>
                
                <div class="w-32 h-32 rounded-full bg-yellow-400 flex items-center justify-center mb-6 shadow-md">
                ...
                <a href="{{ route('guru.detail_materi', $jadwal->id_jadwal) }}" class="w-full bg-blue-500 text-white font-semibold py-3 px-6 rounded-xl text-center hover:bg-blue-600 transition-all transform hover:scale-105 shadow-md hover:shadow-lg flex items-center justify-center space-x-2">
