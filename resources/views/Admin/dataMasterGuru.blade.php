...
<div class="flex flex-col h-full space-y-4 sm:space-y-6">
    ...
    <div class="flex space-x-4">
        <a href="{{ route('admin.data-master.kelas.siswa', $kelas->id_kelas) }}" class="px-8 py-2 bg-white text-slate-700 font-semibold rounded-full border-2 border-blue-200 hover:bg-blue-50 transition-colors">
            Siswa
        </a>
        <a href="{{ route('admin.data-master.kelas.guru', $kelas->id_kelas) }}" class="px-8 py-2 bg-blue-400 text-white font-semibold rounded-full shadow-sm hover:bg-blue-500 transition-colors">
            Guru
        </a>
    ...
    <div class="bg-white p-4 sm:p-6 md:p-8 rounded-2xl shadow-sm grow">
        <h3 class="text-xl font-bold text-blue-600 mb-4 sm:mb-6">Daftar Guru Mengajar</h3>
        ...
        <div class="space-y-4">
            @forelse($guruList as $index => $guru)
                <div class="border-2 border-blue-100 rounded-xl px-4 sm:px-6 py-4">
                    ...
                    <div class="lg:hidden space-y-3">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="font-semibold text-slate-800">{{ $guru->nama_lengkap }}</div>
                                    <div class="text-slate-600 text-sm">{{ $guru->nip }}</div>
                                    <div class="text-slate-600 text-sm">{{ $guru->mataPelajaran->nama_mapel ?? '-' }}</div>
                                </div>
                            </div>
                        </div>
                        <a href="{{ route('admin.data-master.guru.show', $guru->id_guru) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center justify-center space-x-1 transition-colors text-sm w-full">
                            <span>Detail</span>
                        </a>
                    </div>
                    <!-- Desktop Layout -->
                    <div class="hidden lg:grid grid-cols-12 gap-4 items-center">
                        ...
                        <div class="col-span-3 text-slate-600 font-medium text-sm">{{ $guru->nip }}</div>
                        <div class="col-span-3 text-slate-600 font-medium text-sm">{{ $guru->mataPelajaran->nama_mapel ?? '-' }}</div>
                        <div class="col-span-2 text-right">
                            <a href="{{ route('admin.data-master.guru.show', $guru->id_guru) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-lg inline-flex items-center space-x-1 transition-colors text-sm">
                                <span>Detail</span>
                            </a>
                        </div>
                    </div>
            ...
            @empty
                <div class="text-center py-8 text-slate-500">
                    Belum ada guru yang mengajar di kelas ini
                </div>
            @endforelse
    </div>
</div>
...
<div class="flex flex-col sm:flex-row justify-end gap-3">
    <a href="{{ route('admin.data-master.guru.create') }}" class="bg-blue-400 hover:bg-blue-500 text-white px-4 py-2 sm:px-6 sm:py-3 rounded-full font-bold flex items-center justify-center space-x-2 shadow-lg transition-all hover:shadow-xl w-full sm:w-auto">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
