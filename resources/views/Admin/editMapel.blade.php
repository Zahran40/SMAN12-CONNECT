...
<div class="flex flex-col h-full">
    <div class="flex items-center space-x-4 mb-4">
        <a href="{{ route('admin.akademik.mapel.show', $mapel->id_mapel) }}" class="w-12 h-12 flex items-center justify-center bg-blue-100 text-blue-700 rounded-full hover:bg-blue-200 transition-colors" title="Kembali">
    ...
    <div class="bg-white p-8 rounded-2xl shadow-sm">
        ...
        <form action="{{ route('admin.akademik.mapel.update', $mapel->id_mapel) }}" method="POST" class="space-y-5 max-w-2xl">
            @csrf
            @method('PUT')
            
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">Nama Mata Pelajaran</label>
                <input type="text" name="nama_mapel" value="{{ old('nama_mapel', $mapel->nama_mapel) }}" required placeholder="Contoh: Bahasa Indonesia" 
            ...
            <div>
                ...
                <input type="text" name="kode_mapel" value="{{ old('kode_mapel', $mapel->kode_mapel) }}" required placeholder="Contoh: BIND" 
            ...
            <div>
                ...
                <select name="kategori" class="w-full border-2 {{ $errors->has('kategori') ? 'border-red-500' : 'border-blue-200' }} rounded-lg px-4 py-2.5 text-slate-700 focus:outline-none focus:border-blue-500">
                    <option value="Umum" {{ old('kategori', $mapel->kategori) == 'Umum' ? 'selected' : '' }}>Umum (Wajib)</option>
                    <option value="Kelas X" {{ old('kategori', $mapel->kategori) == 'Kelas X' ? 'selected' : '' }}>Kelas X (Fase E)</option>
                    <option value="MIPA" {{ old('kategori', $mapel->kategori) == 'MIPA' ? 'selected' : '' }}>MIPA (Fase F)</option>
                    <option value="IPS" {{ old('kategori', $mapel->kategori) == 'IPS' ? 'selected' : '' }}>IPS (Fase F)</option>
                    <option value="Bahasa" {{ old('kategori', $mapel->kategori) == 'Bahasa' ? 'selected' : '' }}>Bahasa & Budaya</option>
                    <option value="Mulok" {{ old('kategori', $mapel->kategori) == 'Mulok' ? 'selected' : '' }}>Muatan Lokal</option>
                </select>
                @error('kategori')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
