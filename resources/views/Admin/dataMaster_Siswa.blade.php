...
@section('content')
    <div class="flex flex-col space-y-6">
        ...
        <div class="flex space-x-4">
            <a href="{{ route('admin.data-master.index') }}" class="px-8 py-2 bg-white text-slate-700 font-semibold rounded-full border-2 shadow-sm border-blue-200 hover:bg-blue-50 transition-colors">
                Kelas
            </a>
            <a href="{{ route('admin.data-master.list-siswa') }}" class="px-8 py-2 bg-blue-400 text-white font-semibold rounded-full border-2 hover:bg-blue-500 transition-colors">
                Siswa
            </a>
        ...
        <div class="bg-white p-8 rounded-2xl shadow-sm">
            ...
            <div class="space-y-4 max-h-[600px] overflow-y-auto pr-2">
                ...
                <div class="grid grid-cols-12 gap-4 items-center border-2 border-blue-100 rounded-xl px-6 py-4 bg-white">
                    ...
                    <div class="col-span-3 font-semibold text-slate-800 text-sm md:text-base">{{ $siswa->nama_lengkap }}</div>
                    ...
                    <div class="col-span-2 flex flex-col items-end space-y-2">
                        <a href="{{ route('admin.data-master.siswa.show', $siswa->id_siswa) }}" class="bg-blue-400 hover:bg-blue-500 text-white px-4 py-1 rounded-full flex items-center space-x-1 transition-colors text-xs w-20 justify-center">
                         ...
                         <form action="{{ route('admin.data-master.siswa.destroy', $siswa->id_siswa) }}" method="POST" onsubmit="return confirm('Yakin hapus siswa ini?')">
                            @csrf
                            @method('DELETE')
