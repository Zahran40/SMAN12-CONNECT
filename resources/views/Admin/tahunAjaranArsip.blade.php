...
<div class="content-wrapper p-4 sm:p-6">
    ...
    <div class="bg-gray-50 border-2 border-gray-300 rounded-2xl shadow p-5 {{ $index < count($tahunAjaranArsip) - 1 ? 'mb-6' : '' }}">
        <div class="flex justify-between items-center mb-4">
            ...
            <form id="restore-form-{{ $ta->tahun_mulai }}-{{ $ta->tahun_selesai }}" action="{{ route('admin.tahun-ajaran.unarchive-year', [$ta->tahun_mulai, $ta->tahun_selesai]) }}" method="POST">
                @csrf
                <button type="button" onclick="showConfirmModal({
                    title: '🔄 Pulihkan Tahun Ajaran',
                    message: 'Tahun ajaran {{ $ta->tahun_mulai }}/{{ $ta->tahun_selesai }} akan dikembalikan ke daftar aktif.',
                    question: 'Yakin ingin memulihkan dari arsip?',
                    confirmText: 'Ya, Pulihkan',
                    confirmClass: 'flex-1 bg-green-600 text-white font-semibold px-6 py-3 rounded-lg hover:bg-green-700 transition',
                    onConfirm: () => document.getElementById('restore-form-{{ $ta->tahun_mulai }}-{{ $ta->tahun_selesai }}').submit()
