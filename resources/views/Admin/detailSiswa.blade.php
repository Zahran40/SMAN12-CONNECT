...
<div class="flex flex-col space-y-4 sm:space-y-6">
    ...
    <div class="bg-white p-4 sm:p-6 rounded-2xl shadow-sm flex justify-between items-center">
        ...
        <form id="delete-siswa-{{ $siswa->id_siswa }}" action="{{ route('admin.data-master.siswa.destroy', $siswa->id_siswa) }}" method="POST">
            @csrf
            @method('DELETE')
        <button type="button" onclick="showConfirmModal({
            title: '⚠️ Hapus Siswa',
            message: 'Data siswa {{ $siswa->nama_lengkap }} akan dihapus secara permanen.',
            question: 'Yakin ingin menghapus siswa ini?',
            confirmText: 'Ya, Hapus',
            onConfirm: () => document.getElementById('delete-siswa-{{ $siswa->id_siswa }}').submit()
        })" class="bg-red-500 hover:bg-red-600 text-white px-6 py-2.5 rounded-lg flex items-center space-x-2 transition-colors font-medium">
